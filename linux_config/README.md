...
You can visit http://3.81.80.35/ or http://ec2-3-81-80-35.compute-1.amazonaws.com  for the website deployed.
...
### Step 1: Start a new Ubuntu Linux server instance on Amazon EC2 
  ...
  - Check for instance IPv4 public IP - 3.81.80.35
  - we can download a .pem file and connect with following command
    ```
    ssh -i akhil123.pem ubuntu@3.81.80.35
    ``` 
...
- To check port 2200 wether working or not by `ssh -i akhil123.pem -p 2200 ubuntu@3.81.80.35` 
...
### Step 7: Create an SSH key pair for `grader`
  ...
  - Restart SSH: `sudo service ssh restart`
  - On the local machine, cheking if the grader account working or not by running this command : `ssh -i akhil123.pem -p 2200 grader@3.81.80.35`.
...
`sudo git clone https://github.com/akhilkata96/catalog`.
...
- We need to change sqlite to postgresql create_engine in `__init__.py`,`database_setup.py` and `fakeitems.py`,
   ```
...
- Click `Credentials`.
- Create an OAuth Client ID (under the Credentials tab), and add http://3.81.80.35.xip.io and 
http://ec2-3-81-80-35.compute-1.amazonaws.com / as authorized JavaScript 
origins.
- Add http://3.81.80.35.xip.io/login,http://3.81.80.35.xip.io/gconnect,http://3.81.80.35.xip.io/callback
...
### Step 13.2: Set up and enable a virtual host
  ...
  ```
  <VirtualHost *:80>
    ServerName 3.81.80.35.xip.io
    ServerAlias ec2-3-81-80-35.compute-1.amazonaws.com 
    ServerAdmin ubuntu@3.81.80.35
...
- Open your browser to http://3.81.80.35 or http://ec2-3-81-80-35.compute-1.amazonaws.com .
